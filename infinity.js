const textFragments = [
    ...
    "He was a tall man, with nondescript features",
    "or perhaps it was my myopia that made them seem that way",
    "Dressed in gray and carrying a gray suitcase in his hand",
    "he had an unassuming look about him",
    "I saw at once that he was a foreigner",
    "At first, he struck me as old",
    "only later did I realize that I had been misled by his thin blond hair",
    "which was, in a Scandinavian sort of way, almost white",
    "During the course of our conversation, which was not to last an hour",
    "I found out that he came from the Orkneys",
    ...
    "Somewhat pedantically, I replied",
    "In this house are several English Bibles, including the first — John Wiclif's",
    "I also have Cipriano de Valera's, Luther's",
    "which, from a literary viewpoint, is the worst",
    "and a Latin copy of the Vulgate",
    "As you see, it's not exactly Bibles I stand in need of",
    "After a few moments of silence, he said",
    "I don't only sell Bibles",
    ...
    "I noticed that one left-hand page bore the number (let us say) 40,514",
    "and the facing right-hand page 999",
    "I turned the leaf; it was numbered with eight digits",
    "It also bore a small illustration, like the kind used in dictionaries",
    "an anchor drawn with pen and ink, as if by a schoolboy's clumsy hand",
    ...
    "I acquired the book in a town out on the plain",
    "in exchange for a handful of rupees and a Bible",
    "Its owner did not know how to read",
    "I suspect that he saw the Book of Books as a talisman",
    "He was of the lowest caste",
    "nobody but other untouchables could tread his shadow without contamination",
    ...
    "I laid my left hand on the cover",
    "and, trying to put my thumb on the flyleaf, I opened the book",
    "It was useless",
    ...
    "Again I failed",
    "In a voice that was not mine, I barely managed to stammer",
    "This can't be",
    ...
    "Then, as if he were thinking aloud, he said",
    ...
    "Yes, I'm a Presbyterian",
    "My conscience is clear",
    "I am reasonably sure of not having cheated the native",
    "when I gave him the Word of God in exchange for his devilish book",
    "I assured him that he had nothing to reproach himself for",
    "and I asked if he were just passing through this part of the world",
    ...
    "I told him I had a great personal affection for Scotland",
    ...
    "With feigned indifference, I asked",
    "Do you intend to offer this curiosity to the British Museum?",
    "No. I'm offering it to you, he said",
    "and he stipulated a rather high sum for the book",
    "I answered, in all truthfulness, that such a sum was out of my reach",
    ...
    "I'll offer you the amount of my pension check, which I've just collected",
    "and my black-letter Wiclif Bible",
    ...
    "He turned the leaves and studied the title page",
    "with all the fervor of a true bibliophile",
    "It's a deal, he said",
    "It amazed me that he did not haggle",
    "Only later was I to realize that he had entered my house",
    "with his mind made up to sell the book",
    "Without counting the money, he put it away",
    "We talked about India, about Orkney",
    "and about the Norwegian jarls who once ruled it",
    ...
    "but in the end I decided to hide it",
    "behind the volumes of a broken set of The Thousand and One Nights",
    ...
    "The upper corner of the page carried a number, which I no longer recall",
    "elevated to the ninth power",
    ...
    "After studying its frayed spine and covers with a magnifying glass",
    "I rejected the possibility of a contrivance of any sort",
    "The small illustrations, I verified, came two thousand pages apart",
    "I set about listing them alphabetically in a notebook",
    "which I was not long in filling up",
    "Never once was an illustration repeated",
    "At night, in the meager intervals my insomnia granted",
    "I dreamed of the book",
    "Summer came and went",
    "and I realized that the book was monstrous",
    "What good did it do me to think that I",
    "who looked upon the volume with my eyes",
    "who held it in my hands",
    "was any less monstrous?",
    "I felt that the book was a nightmarish object",
    "an obscene thing that affronted and tainted reality itself",
    "I thought of fire",
    "but I feared that the burning of an infinite book",
    "might likewise prove infinite and suffocate the planet with smoke",
    "Somewhere I recalled reading that the best place to hide a leaf is in a forest",
    "Before retirement, I worked on Mexico Street",
    "at the Argentine National Library",
    "which contains nine hundred thousand volumes",
    "I knew that to the right of the entrance",
    "a curved staircase leads down into the basement",
    "where books and maps and periodicals are kept",
    "One day I went there",
    "and, slipping past a member of the staff",
    "and trying not to notice at what height or distance from the door",
...
function createFragments() {
    textFragments.forEach((text, index) => {
        ...
        fragment.addEventListener('click', () => handleFragmentClick(fragment, text, index));
...
function handleFragmentClick(fragment, text, index) {
...
function scatterNearbyFragments(clickedFragment) {
    const clickedRect = clickedFragment.getBoundingClientRect();
    const clickedX = clickedRect.left + clickedRect.width / 2;
    const clickedY = clickedRect.top + clickedRect.height / 2;
    
    fragments.forEach(frag => {
        if (frag.element === clickedFragment) return;
        ...
        const distance = Math.sqrt(
            Math.pow(clickedX - fragX, 2) + 
            Math.pow(clickedY - fragY, 2)
        );
        
        
        if (distance < 300) {
            scatterFragment(frag.element);
        }
    });
}


function scatterFragment(fragment) {
    ...
    setTimeout(() => {
        fragment.style.transition = 'all 0.3s ease';
    }, 800);
}


function shuffleAllFragments() {
    fragments.forEach(frag => {
        if (!frag.element.classList.contains('active')) {
            setTimeout(() => {
                scatterFragment(frag.element);
            }, Math.random() * 2000);
        }
...
setInterval(shuffleAllFragments, 30000);


window.addEventListener('resize', () => {
    fragments.forEach(frag => {
        const currentX = parseInt(frag.element.style.left);
        const currentY = parseInt(frag.element.style.top);
        
        if (currentX > window.innerWidth - 300) {
            frag.element.style.left = (window.innerWidth - 300) + 'px';
        }
        if (currentY > window.innerHeight - 200) {
            frag.element.style.top = (window.innerHeight - 200) + 'px';
        }
    });
});
